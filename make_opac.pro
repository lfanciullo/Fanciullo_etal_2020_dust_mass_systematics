...
for j = 0, nmat-1 do begin
   ...
   for iwl = 0, nwl_fromfile-1 do begin
      ...
      if count_lowT then mac_temp[iwl, lowT] = mac_fromfile[iwl, 0]
      if count_hiT then mac_temp[iwl, hiT] = mac_fromfile[iwl, nT_fromfile-1]
   endfor
   ...
   if plot then begin
      ...
      longwl = where(wl_fromfile GT 100.)
      vlongwl = where(wl_fromfile GT 500.)
      Tsubs = [0, 2, 4] & lstylesubs = [0, 5, 1]
      ...
      if keyword_set(saveplot) then begin
         set_plot, 'PS'
         device, filename = fold_pics + 'Smoothing_systematics_' +  material[j] + '.eps', /color, /encapsulated
      endif else begin
         window, /free
      endelse
      xlims = [80., 2000.]
      ...
      for i = 0, n_elements(Tsubs)-1 do oplot, wl_fromfile[longwl], 100*(mac_temp_smooth[longwl,Tsubs[i]]/mac_temp[longwl,Tsubs[i]]-1) $
